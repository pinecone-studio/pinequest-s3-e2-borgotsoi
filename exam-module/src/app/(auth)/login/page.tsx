...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    const activeCode = role === "teacher" ? teacherCode : studentCode;
    if (!activeCode.trim() || !password.trim()) {
    ...
    try {
      ...
      const testUser = {
        code: role === "teacher" ? "teacher123" : "B252270158",
        password: "password123",
      };
      if (activeCode.trim() !== testUser.code || password !== testUser.password) {
        ...
        <div className={styles.logoArea}>
          <div className={styles.logoMark}>
            <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
              <polygon points="18,2 34,10 34,26 18,34 2,26 2,10" fill="none" stroke="#FFC000" strokeWidth="1.5" />
              <polygon points="18,8 28,13 28,23 18,28 8,23 8,13" fill="none" stroke="#FFC000" strokeWidth="0.8" opacity="0.5" />
        ...
        <div className={styles.roleRow}>
          {(["teacher", "student"] as Role[]).map((r) => (
            ...
            >
              ...
              {r === "teacher" ? "Багш" : "Оюутан"}
              {role === r && <span className={styles.roleActiveDot} />}
            </button>
          ))}
        </div>
        ...
        <form onSubmit={handleLogin} className={styles.form}>
          ...
          <div className={styles.fieldWrap}>
            <label className={styles.label}>
              {role === "teacher" ? "Багшийн код" : "Сурагчийн код"}
            </label>
            <div className={styles.inputWrap}>
              ...
              <Input
                type="text"
                placeholder={role === "teacher" ? "teacher123" : "B252270158"}
                value={role === "teacher" ? teacherCode : studentCode}
                onChange={(e) =>
                  role === "teacher"
